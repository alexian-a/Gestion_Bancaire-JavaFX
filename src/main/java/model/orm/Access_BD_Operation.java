...
public class Access_BD_Operation {
    ...
    /**
     * Recherche de toutes les opérations d'une date en particulier.
     *
     * @throws DatabaseConnexionException Erreur de connexion
     */
    public ArrayList<Operation> getOperationsToday() throws DataAccessException, DatabaseConnexionException {

        ArrayList<Operation> alResult = new ArrayList<>();

        try {
            Connection con = LogToDatabase.getConnexion();
            String query = "SELECT * FROM OPERATION WHERE DATEOP < ? AND DATEOP >= ? ORDER BY DATEOP DESC";
            ...
            Date tomorrowSqlDate = Date.valueOf(tomorrow);

            pst.setDate(1, tomorrowSqlDate);
            pst.setDate(2, todaySqlDate);
     ...
     * @throws RowNotFoundOrTooManyRowsException La requête renvoie plus de 1
     * ligne
     * @throws DataAccessException Erreur d'accès aux données (requête mal
     * formée ou autre)
     * @throws DatabaseConnexionException Erreur de connexion
        ...
        try {
            ...
            if (res != 0) { // Erreur applicative
                throw new ManagementRuleViolation(Table.Operation, Order.INSERT,
                        "Erreur de règle de gestion : découvert autorisé dépassé ou compte à débiter cloturé", null);
            }
        } catch (SQLException e) {
     ...
     *
     * Se fait par procédure stockée : - Enregistre l'opération <BR />
     * - Met à jour le solde du compte. <BR />
     *
     * @param idNumCompte compte débité
     * @param montant montant débité
    ...
    public void insertDebitExceptionnel(int idNumCompte, double montant, String typeOp)
        ...
        try {
            ...
            if (res != 0) { // Erreur applicative
                throw new ManagementRuleViolation(Table.Operation, Order.INSERT,
                        "Erreur de règle de gestion : compte à débiter cloturé", null);
    ...
    public void insertCredit(int idNumCompte, double montant, String typeOp)
        ...
        try {
            ...
            if (res != 0) { // Erreur applicative
                throw new ManagementRuleViolation(Table.Operation, Order.INSERT,
                        "Erreur de règle de gestion : compte à créditer cloturé", null);
            }
        } catch (SQLException e) {
            throw new DataAccessException(Table.Operation, Order.INSERT, "Erreur accès", e);
        }
    ...
    /**
     * Permet d'insérer un virement en utilisant insertDebit et insertCredit
     *
     * @param idNumCompte compte qui vire l'argent (débité)
     * @param montant montant viré
     * @param typeOp type d'opération
     * @param idNumCompte2 compte à qui on vire l'argent (crédité)
     ...
     *
     */
    public void insertVirement(int idNumCompte, double montant, String typeOp, int idNumCompte2) throws DatabaseConnexionException, ManagementRuleViolation, DataAccessException {
        insertCredit(idNumCompte2, montant, typeOp);
        insertDebit(idNumCompte, montant, typeOp);

    }

    /**
     * Permet d'insérer un emprunt
     *
     * @param idEmprunt numéro de l'emprunt
     * @param tauxInteret taux d'intérêt de l'emprunt
     * @param montant montant emprunté
     * @param duree durée de l'emprunt
     * @param dateEmp date de l'emprunt
     * @param idNumClient numéro du client qui emprunte
     * @throws DatabaseConnexionException
     * @throws ManagementRuleViolation
     * @throws DataAccessException
     *
     ...
     *
     */
    public void insertEmprunt(int idEmprunt, double tauxInteret, double montant, int duree, Date dateEmp, int idNumClient) throws DatabaseConnexionException, ManagementRuleViolation, DataAccessException {
        try {
            Connection con = LogToDatabase.getConnexion();
            CallableStatement call;

            String q = "INSERT INTO EMPRUNT VALUES (?, ?, ?, ?, ?, ? )";
            call = con.prepareCall(q);
            // Paramètres in
            call.setInt(1, idEmprunt);
            call.setDouble(2, tauxInteret);
            call.setDouble(3, montant);
            call.setInt(4, duree);
            call.setDate(5, dateEmp);
            call.setInt(6, idNumClient);
