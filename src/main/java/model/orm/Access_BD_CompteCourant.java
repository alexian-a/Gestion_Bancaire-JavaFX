...
public class Access_BD_CompteCourant {
     ...
     *
     * @param cc le compte sélectionné
     * @throws DatabaseConnexionException
     * @throws RowNotFoundOrTooManyRowsException
     * @throws ManagementRuleViolation
     */
    public void clotureCompteCourant(CompteCourant cc) throws DataAccessException, DatabaseConnexionException, RowNotFoundOrTooManyRowsException, ManagementRuleViolation {
        try {

            if (cc.solde != 0) {
                throw new ManagementRuleViolation(Table.CompteCourant, Order.UPDATE,
                        "Erreur de règle de gestion : solde non nul", null);
            }
     ...
     * <p>
     * cc.idNumCompte (clé primaire) doit exister seul cc.debitAutorise est mis
     * à jour cc.solde non mis à jour (ne peut se faire que par une opération)
     * cc.idNumCli non mis à jour (un cc ne change pas de client)
     *
     * @param cc IN cc.idNumCompte (clé primaire) doit exister seul
     * @throws RowNotFoundOrTooManyRowsException La requête modifie 0 ou plus de
     *                                           1 ligne
     * @throws DataAccessException               Erreur d'accès aux données (requête mal
     *                                           formée ou autre)
     * @throws DatabaseConnexionException        Erreur de connexion
     * @throws ManagementRuleViolation           Erreur sur le solde courant par rapport
     *                                           au débitAutorisé (solde < débitAutorisé)
     */
     ...
     * @throws RowNotFoundOrTooManyRowsException La requête insère 0 ou plus de
     *                                           1 ligne
     * @throws DataAccessException               Erreur d'accès aux données (requête mal
     *                                           formée ou autre)
     * @throws DatabaseConnexionException        Erreur de connexion
     ...
     *
     * @param idCompte id du compte à supprimer
     * @throws RowNotFoundOrTooManyRowsException La requête supprime 0 ou plus de 1
     *                                           ligne
     * @throws DataAccessException               Erreur d'accès aux données (requête
     *                                           mal formée ou autre)
     * @throws DatabaseConnexionException        Erreur de connexion
     */
    public void supprimerCompteCourant(int idCompte)
            throws RowNotFoundOrTooManyRowsException, DataAccessException, DatabaseConnexionException {
        try {
            Connection con = LogToDatabase.getConnexion();
            ...
            String deleteOperationQuery = "DELETE FROM Operation WHERE idNumCompte = ?";
            try (PreparedStatement pstOperation = con.prepareStatement(deleteOperationQuery)) {
                pstOperation.setInt(1, idCompte);
                pstOperation.executeUpdate();
            }

            // Delete CompteCourant
            String deleteCompteCourantQuery = "DELETE FROM CompteCourant WHERE idNumCompte = ?";
            try (PreparedStatement pstCompteCourant = con.prepareStatement(deleteCompteCourantQuery)) {
                pstCompteCourant.setInt(1, idCompte);
                int result = pstCompteCourant.executeUpdate();

                if (result != 1) {
                    con.rollback();
                    throw new RowNotFoundOrTooManyRowsException(Table.Employe, Order.DELETE,
                            "Problème sur le delete, la ligne n'a pas été trouvée", null, result);
                }
            }
