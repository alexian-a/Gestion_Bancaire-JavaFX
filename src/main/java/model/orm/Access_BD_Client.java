...
public class Access_BD_Client {
	 ...
	 * @param idComptes tableau des id des comptes à supprimer
	 ...
	 * @throws DatabaseConnexionException        Erreur de connexion
	 */
	public void supprimerClient(int idClient, int[] idComptes)
			throws RowNotFoundOrTooManyRowsException, DataAccessException, DatabaseConnexionException {
		try {
			Connection con = LogToDatabase.getConnexion();
			...
			String deleteCompteCourantQuery = "DELETE FROM CompteCourant WHERE idNumCli = ?";
			try (PreparedStatement pstCompteCourant = con.prepareStatement(deleteCompteCourantQuery)) {
				pstCompteCourant.setInt(1, idClient);
				pstCompteCourant.executeUpdate();
			}

			// Delete Client
			String deleteClientQuery = "DELETE FROM Client WHERE idNumCli = ?";
			try (PreparedStatement pstClient = con.prepareStatement(deleteClientQuery)) {
				pstClient.setInt(1, idClient);
				int result = pstClient.executeUpdate();
